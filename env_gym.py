...
class MMCEnv(Env):
    def __init__(self,path,fs,model,knowledge="detector"):
        ...
        self.obs_low = np.full(self.obs_shape,-np.inf) # Por ahora no hay una cota definida 
        self.obs_high = np.full(self.obs_shape,np.inf)
        self.observation_space = spaces.Box(low=self.obs_low,high=self.obs_high,shape=self.obs_shape)
        self.action_space = spaces.Box(low=np.array([-50.0]),high=np.array([50.0]),shape=(1,))
    ...
    def reset(self):
        ...
        self.eng.set_param(self._model,'SimulationCommand','start','SimulationCommand','start',nargout=0)
    ...
    def step(self, action):
        ...
        t_act = self.eng.eval("get_param(bdroot, 'SimulationTime')",nargout=0) 
        self.eng.set_param('{}/Temp'.format(self._model),'value',str(t_act),nargout=0)
    ...
    def system_obs(self):
        ...
        return np.concatenate(error[0],i_arm,m_i)
    ...
    def detector_obs(self):
        ...
        return np.concatenate(error[0],index[0])        
