...
class MMCEnv(Env):
    ...
    def step(self, action):
        ...
        obs = self.obs_func()
        t = self.eng.eval("tout(end)")
        idx = self.eng.eval("i(end,:)")
    ...
    def system_obs(self):
        ...
        error = np.array(self.eng.eval("error(end,:)"))
        i_arm = np.array([self.eng.eval("i_aU(end)")])
        m_i = np.array([self.eng.eval("m1(end)")])
    ...
    def detector_obs(self):
        ...
        error = np.array(self.eng.eval("error(end,:)"))
        index = np.array(self.eng.eval("index(end,:)"))
    ...
    def get_info(self):
        vout = np.array(self.eng.eval("vout"))
        indices = np.array(self.eng.eval("index"))
        actions = np.array(self.eng.eval("action"))
